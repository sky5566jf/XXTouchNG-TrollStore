local io  = require "io"
local os  = require "os"
local posix = require "posix.init"
local Log = require "log"

local make_attr, color_writeln, COLORS
local fifo_path = "/private/var/tmp/ch.xxtou.DebugWindow.FIFO"

function FIFO(callback)
  local fifo_fd = posix.open(fifo_path, posix.O_WRONLY | posix.O_NONBLOCK, 'rw-r--r--')
  if fifo_fd ~= nil then
    local fifo = posix.fdopen(fifo_fd, "w")
    callback(fifo)
    fifo:close()
  end
end
...
if not COLORS then -- ansicolors
  ...
  if ok and (type(c) == 'table') then
    if c.black then -- version above 1.0.2
      ...
      color_writeln = function (attr, text) FIFO(function (fifo) fifo:write(attr, text, reset, '\n') end) end
  ...
  if ok and (type(c) == 'function') then
    ...
    make_attr = function (F, B) return fore[F] .. back[B] end
    color_writeln = function (attr, text) FIFO(function (fifo) fifo:write(c(attr .. text), '\n') end) end
  end
end
